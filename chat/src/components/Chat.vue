  ...
  <div v-if="!isLoading" class="chats">
    <div class="sidebar left-bar">
      ...
      <ul class="chat-list">
        ...
        ></ChatListItem>
        <!-- :last-message="chat.messages[chat.messages.length - 1]" - это к ChatListItem -->
      </ul>
    </div>
    <div class="sidebar right-bar">
      <div class="top-bar">
        <div class="chosen-chat-info">
          ...
          <p v-if="selectedChat !== -1" class="chosen-chat-info__number-of-people">
      ...
      <form v-if="selectedChat !== -1" class="message-form" @submit.prevent="sendMsg">
        <textarea
          type="text"
          v-model="newMessage"
        ...
        if (response.data.length !== 0)
          this.chats = response.data.map((chat) => {
            return {
              id: chat.id,
              title: chat.name,
              numberOfMembers: chat.users.length,
              // messages: [
              //   {
              //     id: 9,
